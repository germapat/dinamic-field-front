  ...
  <div>
    <q-table
      ...
      <template slot="top-right" slot-scope="props">
        <q-field icon="find_in_page">
          <q-input
            ...
            :after="[{icon: 'search', content: true, handler () { search() }}]"
          />
        </q-field>
      </template>
      ...
      <q-td v-if="datatable.hasOwnProperty('actions') && datatable.actions ? true : datatable.hasOwnProperty('actions') && !datatable.actions ? false : true" slot="body-cell-manage" slot-scope="props" :props="props">
        ...
        <q-btn
          v-if="$q.localStorage.get.item('permissions').indexOf('db.delete_custom_') > -1 || $q.localStorage.get.item('permissions').indexOf('pyctivex.executive') > -1 && props.row.type != 'default'"
          color="negative"
          flat
          round
          icon="delete"
